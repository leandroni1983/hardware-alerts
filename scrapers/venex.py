...
class VenexScraper(BaseScraper):
    def __init__(
        ...
        shop_id: str = "venex",
        shop_name: str = "Venex",
        base_url: str = "https://www.venex.com.ar",
    ):
        super().__init__(shop_id, shop_name, base_url)
        self.logger = logging.getLogger("scraper.venex")

    async def get_category_urls(self) -> List[Tuple[str, str]]:
        return [
            (
                f"{self.base_url}/resultado-busqueda.htm?keywords=placa+de+video",
                "Placas de video",
            ),
            (
                f"{self.base_url}/resultado-busqueda.htm?keywords=procesadores",
                "Procesadores",
            ),
        ]

    @staticmethod
    def _parse_price_from_attrs(attrs: dict | None) -> int | None:
        if not attrs:
            return None
        # common patterns in ecommerce markup
        for k in (
            "data-price",
            "data-final-price",
            "data-amount",
    ...
    async def scrape_category(
        ...
        while next_url and page_num < max_pages:
            ...
            for c in cards:
                try:
                    title = None
                    for tsel in ["h2", "h3", ".title", ".nombre", ".product-title"]:
                    ...
                    # 1) Try explicit nodes + attributes inside the product-box
                    ...
                    for psel in [
                        ...
                        if pel:
                            ...
                            try:
                                ...
                                attrs = None
                            p_attr = self._parse_price_from_attrs(attrs)
                            if p_attr:
                                price = p_attr if price is None else max(price, p_attr)

                            # content/value are common for meta/input
                            try:
                                content = await pel.get_attribute("content")
                                if content:
                                    price_texts.append(content)
                            except Exception:
                                pass
                            try:
                                value = await pel.get_attribute("value")
                                if value:
                                    price_texts.append(value)
                            except Exception:
                                pass

                    # attributes on the card itself
                    try:
                        c_attrs = await c.evaluate(
                            """el => {
                              const out = {};
                              for (const a of el.attributes) out[a.name] = a.value;
                              return out;
                        ...
                        c_attrs = None
                    p_attr = self._parse_price_from_attrs(c_attrs)
                    if p_attr:
                        price = p_attr if price is None else max(price, p_attr)
                    ...
                    # 2) Fallback: parse from whole card text (Venex often renders price without stable class)
                    if price is None:
                        try:
