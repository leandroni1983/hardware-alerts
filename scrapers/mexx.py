...
class MexxScraper(BaseScraper):
    def __init__(
        ...
        shop_id: str = "mexx",
        shop_name: str = "Mexx",
        base_url: str = "https://www.mexx.com.ar",
    ):
        super().__init__(shop_id, shop_name, base_url)
        self.logger = logging.getLogger("scraper.mexx")

    async def get_category_urls(self) -> List[Tuple[str, str]]:
        return [
            (f"{self.base_url}/buscar/?p=placa+de+videos", "Placas de video"),
            (f"{self.base_url}/buscar/?p=procesadores", "Procesadores"),
        ]
        ...
        while next_url and page_num < max_pages:
            ...
            for c in cards:
                try:
                    ...
                    # 1) Prefer explicit title nodes if present
                    ...
                    # 2) Fallback: take from link attributes inside the card
                    if not title:
                        a_title = await c.query_selector("a[title]")
                        if a_title:
                            t = await a_title.get_attribute("title")
                            title = (t or "").strip() or None

                    # 3) Fallback: take from link text (but ignore "EN STOCK")
                    if not title:
                        a = await c.query_selector("a")
                        if a:
                            t = (await a.inner_text()).strip()
                            if t and t.lower() not in {"en stock", "sin stock"}:
                                title = t

                    # 4) Fallback: derive name from href slug when link text is not the product name
                    if not title:
                        a_href = await c.query_selector("a[href]")
                        href_v = await a_href.get_attribute("href") if a_href else None
                        if href_v:
                            # example:
                            # /productos-rubro/placas-de-video/50617-placa-de-video-geforce-rtx-3060-12gb-asus-dual-oc-v2.html
                            slug = href_v.split("?")[0].rstrip("/").split("/")[-1]
                            slug = slug.replace(".html", "")
                            slug = re.sub(r"^\d+-", "", slug)  # drop leading id-
                            slug = slug.replace("-", " ").strip()
                            title = slug or None

                    # 5) Final fallback: use full card text (best-effort)
                    if not title:
                        t = (await c.inner_text()).strip()
                        title = t.split("\n")[0].strip() if t else None
